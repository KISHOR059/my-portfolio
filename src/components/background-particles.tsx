...
export function BackgroundParticles() {
  ...
  return (
    <div className="pointer-events-none fixed inset-0 z-0 overflow-hidden" aria-hidden="true">
      ...
      <motion.div className="absolute -left-40 top-[22%] size-[28rem] rounded-full bg-violet-600/[.055] blur-[120px]" animate={reducedMotion ? undefined : { x: [0, 180, 40, 0], y: [0, -60, 80, 0], scale: [1, 1.18, 0.9, 1] }} transition={{ duration: 25, repeat: Infinity, ease: "easeInOut" }} />
      <motion.div className="absolute -right-44 top-[55%] size-[32rem] rounded-full bg-cyan-500/[.045] blur-[130px]" animate={reducedMotion ? undefined : { x: [0, -160, -20, 0], y: [0, 80, -50, 0], scale: [1, 0.9, 1.2, 1] }} transition={{ duration: 29, repeat: Infinity, ease: "easeInOut" }} />
      <motion.div className="absolute inset-0 opacity-[.035] [background-image:linear-gradient(rgba(255,255,255,.7)_1px,transparent_1px),linear-gradient(90deg,rgba(255,255,255,.7)_1px,transparent_1px)] [background-size:80px_80px] [mask-image:linear-gradient(to_bottom,black,transparent_85%)]" style={{ y: farStarsY }} />

      <motion.div className="absolute inset-0" style={{ y: nearStarsY }}>
        {stars.map((star, index) => (
          <motion.span
            ...
            animate={reducedMotion ? undefined : { opacity: [0.14, index % 6 === 0 ? 1 : 0.72, 0.14], scale: [1, index % 8 === 0 ? 2.2 : 1.55, 1] }}
