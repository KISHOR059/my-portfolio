  ...
  return (
    <footer className="relative z-10 px-4 pb-4 pt-5 sm:px-6 sm:pt-6">
      <div className="-mx-4 w-[calc(100%+2rem)] overflow-hidden border-y border-white/[.08] bg-white/[.025] shadow-[0_24px_80px_rgba(0,0,0,.25),inset_0_1px_0_rgba(255,255,255,.045)] sm:-mx-6 sm:w-[calc(100%+3rem)]">
        ...
        <div className="flex w-full flex-col items-center gap-5 p-4 text-center sm:p-5 lg:flex-row lg:items-center lg:justify-between lg:text-left">
          <a href="#home" className="group flex items-center gap-3" aria-label="Return to home">
            ...
            <span><span className="block text-sm font-bold tracking-tight text-white">kishor<span className="text-cyan-300">.dev</span></span><span className="mt-0.5 block text-[10px] text-slate-300">Software Engineer · India</span></span>
          </a>
          <nav className="flex flex-wrap items-center justify-center gap-x-4 gap-y-2 sm:gap-x-5" aria-label="Footer navigation">
            {navigation.map((item) => <a key={item} href={item === "Home" ? "#home" : "#" + item.toLowerCase()} className="text-[11px] text-slate-300 transition-colors hover:text-white">{item}</a>)}
          </nav>
        ...
        <div className="flex w-full flex-col items-center justify-between gap-1.5 border-t border-white/[.06] px-4 py-3 text-center sm:flex-row sm:px-5 sm:text-left">
          <p className="text-[11px] font-medium text-slate-200">Designed &amp; engineered with intention</p>
          <p className="font-mono text-[11px] font-medium text-slate-200">© {new Date().getFullYear()} Kishor</p>
        </div>
      </div>
    </footer>
  );
