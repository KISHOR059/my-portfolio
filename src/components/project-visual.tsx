...
function EvChargingVisual({ active }: { active: boolean }) {
  const pins = ["left-[19%] top-[29%]", "left-[53%] top-[18%]", "left-[72%] top-[48%]"];

  return (
    <div className="visual-window overflow-hidden">
      <div className="flex items-center justify-between"><div className="flex items-center gap-2"><motion.span animate={active ? { y: [0, -2, 0], opacity: [.78, 1, .78] } : undefined} transition={{ duration: 2.2, repeat: Infinity }} className="grid size-7 place-items-center rounded-lg bg-cyan-400/10 text-cyan-300"><Zap className="size-3.5" /></motion.span><div><p className="text-[9px] font-semibold text-white">EV Charge Map</p><p className="text-[7px] text-slate-500">Stations near you</p></div></div><span className="inline-flex items-center gap-1.5 rounded-full bg-cyan-400/[.07] px-2 py-1 text-[7px] text-cyan-300"><motion.span animate={active ? { opacity: [.4, 1, .4] } : undefined} transition={{ duration: 1.6, repeat: Infinity }} className="size-1 rounded-full bg-cyan-300" />12 available</span></div>
      <div className="relative mt-3 h-28 overflow-hidden rounded-xl border border-cyan-300/10 bg-[#081225]">
        ...
        {pins.map((position, index) => <motion.span key={position} className={cn("absolute grid size-6 -translate-x-1/2 -translate-y-1/2 place-items-center rounded-full border border-cyan-200/25 bg-[#0c1930] text-cyan-300 shadow-[0_0_16px_rgba(34,211,238,.25)]", position)} animate={active ? { y: [0, -3, 0] } : undefined} transition={{ duration: 2 + index * .35, delay: index * .16, repeat: Infinity }}><motion.span animate={active ? { scale: [1, 1.8], opacity: [.25, 0] } : undefined} transition={{ duration: 1.8, delay: index * .3, repeat: Infinity }} className="absolute inset-0 rounded-full border border-cyan-300/35 opacity-40" /><MapPin className="size-3" /></motion.span>)}
...
function BharatNetVisual({ active }: { active: boolean }) {
  return (
    <div className="visual-window overflow-hidden">
      <div className="flex items-center justify-between"><div className="flex items-center gap-2"><motion.span animate={active ? { scale: [.9, 1.1, .9], opacity: [.8, 1, .8] } : undefined} transition={{ duration: 2.4, repeat: Infinity }} className="grid size-7 place-items-center rounded-lg bg-blue-400/10 text-blue-300"><Network className="size-3.5" /></motion.span><div><p className="text-[9px] font-semibold text-white">BharatNet / Network Operations</p><p className="mt-0.5 text-[7px] text-slate-500">National fiber infrastructure</p></div></div><span className="inline-flex items-center gap-1.5 rounded-full bg-emerald-400/10 px-2 py-1 text-[7px] text-emerald-300"><motion.span animate={active ? { scale: [.8, 1.25, .8], opacity: [.45, 1, .45] } : undefined} transition={{ duration: 1.7, repeat: Infinity }} className="size-1 rounded-full bg-emerald-400 shadow-[0_0_8px_#34d399]" />Live network</span></div>
...
function AetherMindVisual({ active }: { active: boolean }) {
  const tasks = [
  ...
  return (
    <div className="visual-window overflow-hidden">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2"><motion.span animate={active ? { scale: [1, 1.12, 1], opacity: [.8, 1, .8] } : undefined} transition={{ duration: 2.2, repeat: Infinity }} className="grid size-7 place-items-center rounded-lg bg-violet-400/10 text-violet-300"><Sparkles className="size-3.5" /></motion.span><div><p className="text-[9px] font-semibold text-white">AetherMind</p><p className="text-[7px] text-slate-500">AI productivity workspace</p></div></div>
        <span className="inline-flex items-center gap-1.5 rounded-full border border-violet-300/15 bg-violet-400/[.07] px-2 py-1 text-[7px] text-violet-200"><motion.span animate={active ? { opacity: [.45, 1, .45] } : undefined} transition={{ duration: 1.6, repeat: Infinity }} className="size-1 rounded-full bg-violet-300 shadow-[0_0_7px_#a78bfa]" />AI online</span>
      </div>
...
export function ProjectVisual({ project, hovered }: { project: Project; hovered: boolean }) {
  const mobile = useMediaQuery("(max-width: 767px)");
  const reducedMotion = useReducedMotion();
  const visualRef = useRef<HTMLDivElement>(null);
  const inView = useInView(visualRef, { margin: "100px 0px" });
  const active = hovered && !mobile && !Boolean(reducedMotion) && inView;
  return (
    <div ref={visualRef}>
      {project.visual === "commerce" ? <EvChargingVisual active={active} /> : project.visual === "aiProductivity" ? <AetherMindVisual active={active} /> : <BharatNetVisual active={active} />}
    </div>
  );
}
