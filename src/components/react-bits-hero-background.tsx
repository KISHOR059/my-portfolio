...
export function ReactBitsHeroBackground({ accentColor = "#A855F7" }: { accentColor?: string }) {
  ...
  return (
    <div ref={backgroundRef} className="pointer-events-none absolute inset-0 overflow-hidden" style={{ "--hero-accent-rgb": hexToRgb(accentColor) } as CSSProperties} aria-hidden="true">
      <div className="absolute inset-0 bg-[radial-gradient(circle_at_50%_-15%,rgba(var(--hero-accent-rgb),.16),transparent_40%),linear-gradient(to_bottom,#050816_0%,#070817_58%,#050816_100%)]" />
      ...
      >
        <motion.div
          className="absolute inset-0 rounded-full bg-[radial-gradient(circle,rgba(var(--hero-accent-rgb),.78)_0%,rgba(var(--hero-accent-rgb),.42)_34%,rgba(var(--hero-accent-rgb),.13)_58%,transparent_74%)] blur-[62px] mix-blend-screen"
          animate={animateBackground ? {
